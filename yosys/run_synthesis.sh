...
Options:
    ...
    --open              Open Yosys
    --flist             Regenerate flist (src/mcu-soc.flist)

Environment Variables:
    TOP_DESIGN          Top module name (default: ${TOP_DESIGN})

Inputs:
    - RTL file list: src/mcu-soc.flist
...
generate_flist() {
    run_cmd "echo [INFO][Bender] Generate src/mcu-soc.flist"
    run_cmd "bender \
        script flist-plus \
        -t asic \
        -t ihp13 \
    ...
    run_cmd "sed -i 's|${MCU_ROOT}|..|g' src/mcu-soc.flist"

    run_cmd "echo [INFO][Bender] File list generated: src/mcu-soc.flist"
}
